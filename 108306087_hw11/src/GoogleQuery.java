...

{
	...
	public GoogleQuery(String searchKeyword) throws IOException
	{
		...
		this.url = "http://www.google.com/search?q=附中"+searchKeyword+"&oe=utf8&num=10";
...
//				e.printStackTrace();
		...
		for(WebNode node : m.getNode()) {
			...
			*/
			one.add(node.webPage.name);
			two.add(node.webPage.url);
			three.add("Score : "+ node.nodeScore);
		...
		String[][] qqq = new String[one.size()][3];
		
		for(int i = 0; i < one.size(); i++) {
			/*
			if(one.get(i).equals(null)||one.get(i).equals("")) {
